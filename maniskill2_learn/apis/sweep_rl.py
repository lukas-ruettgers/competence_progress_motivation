...
parser = argparse.ArgumentParser(
    description="Simple script sweeping over a configuration file"
)
...
config_file = None 
run_args = None
num_sweep_agents = None
count_per_agent = None

# Get config variables from file
with open(args.config, 'r') as f:
    exec(f.read())

config_file = run_args[0]
assert sweep_configuration is not None, "Config file must contain sweep_configuration variable"

run_args = [] if run_args is None else run_args
num_sweep_agents = 1 if num_sweep_agents is None else num_sweep_agents
count_per_agent = 5 if count_per_agent is None else count_per_agent
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="When work-dir is None, we will use cfg.agent_cfg.type/config_name or config_name/cfg.agent_cfg.type as work-dir",
    )
...
def build_work_dir(args, cfg):
    if is_null(args.work_dir):
        ...
        if args.with_cfg.agent_cfg.type:
            if args.cfg.agent_cfg.type_first:
                args.work_dir = osp.join(root_dir, cfg.agent_cfg.type, folder_name)
            else:
                args.work_dir = osp.join(root_dir, folder_name, cfg.agent_cfg.type)
        else:
        ...
        args.work_dir = osp.join(args.work_dir, cfg.agent_cfg.type)
    ...
    if args.clean_up:
        if (
            args.evaluation
            or args.auto_resume
            or (is_not_null(args.resume_from) and os.path.commonprefix(args.resume_from) == args.work_dir)
        ):
            colored_print(
...
def get_python_env_info(args, cfg):
    ...
    if args.gpu_ids is None:
        args.gpu_ids = []
    if len(args.gpu_ids) == 1 and args.gpu_ids[0] < 0:
        args.gpu_ids = [find_gpu()]
...
def main_rl(rollout, evaluator, replay, args, cfg, expert_replay=None, recent_traj_replay=None):
    ...
    if not args.evaluation:
        ...
        if is_not_null(evaluator):
            # For RL
            lens, rewards, finishes = evaluator.run(agent, work_dir=args.work_dir, **cfg.eval_cfg)
...
def run_one_process(rank, world_size, args, cfg):
    ...
    if is_not_null(cfg.replay_cfg) and (
        not args.evaluation or (args.reg_loss and cfg.replay_cfg.get("buffer_filenames", None) is not None)
    ):
    ...
    if is_not_null(cfg.eval_cfg) and rank == 0:
        ...
        eval_cfg = cfg.eval_cfg
        # Evaluation environment setup can be different from the training set-up. (Like early-stop or object sets)fd
...
def main(config):
    ...
    build_work_dir(args, cfg)
    find_checkpoint(args, cfg)
    get_python_env_info(args, cfg)
    ...
    logger_name = (
        ...
        if isinstance(cfg.env_cfg, dict) and is_not_null(cfg.env_cfg.env_name)
        else cfg.agent_cfg.type
    )
...
if __name__ == "__main__":
    ...
    sweep_id = wandb.sweep(sweep=sweep_configuration, project="her-dreamer")
    for id in range(num_sweep_agents):
        print(f"Start {id} agent!")
        wandb.agent(sweep_id=sweep_id, function=main, count=count_per_agent)
