...
class PPO(BaseAgent):
    def __init__(
        ...
        **kwargs,
        ...
        self.regress_visual_state = (
            visual_state_coeff > 0 and visual_state_mlp_cfg is not None and "visual_state" in obs_shape
        )
    ...
    def compute_critic_loss(self, samples):
        ...
        values = self.critic(samples["obs"], episode_dones=samples["episode_dones"], save_feature=True)
    ...
    def update_parameters(self, memory, updates, rollout=None, with_v=False):
        ...
        if self.num_mini_batch < 0:
            max_samples = (
                GDict(len(memory)).allreduce(op="MAX", device=self.device, wrapper=False)
                if world_size > 1
                else len(memory)
            )
        ...
        def run_over_buffer(epoch_id, mode="v"):
            ...
            for samples in memory.mini_batch_sampler(
                self.batch_size, drop_last=True, auto_restart=True, max_num_batches=num_mini_batch
            ):
