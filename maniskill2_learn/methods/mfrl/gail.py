...
class GAIL(BaseAgent):
    ...
    def update_discriminator_helper(self, expert_replay, recent_traj_replay):
        ...
        recent_traj_out = torch.sigmoid(
            self.discriminator(recent_traj_sampled_batch["obs"], recent_traj_sampled_batch["actions"])
        )
    ...
    def update_parameters(self, memory, updates, expert_replay):
        ...
        with torch.no_grad():
            discriminator_rewards = -torch.log(
                torch.sigmoid(self.discriminator(sampled_batch["obs"], sampled_batch["actions"]))
            )
            assert sampled_batch["rewards"].size() == discriminator_rewards.size()
            old_rewards = sampled_batch["rewards"]
            sampled_batch["rewards"] = (
                self.env_reward_proportion * old_rewards + (1 - self.env_reward_proportion) * discriminator_rewards
            )
            if self.clip_reward:
