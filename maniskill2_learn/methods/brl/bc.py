    ...
    def compute_regression_loss(self, pred_dist, pred_action, target_action, mask=None):
        ...
        if hasattr(F, self.loss_type):
            ...
            if self.actor.head.num_heads == 1:
                actor_loss = (
                    getattr(F, self.loss_type)(pred_action, target_action, reduction="none").mean(-1) * mask
                ).sum() / mask.sum()
            else:
                # Mixture of expert
                ...
                error = (
                    getattr(F, self.loss_type)(all_mean, target_action[..., None, :], reduction="none").mean(-1)
