...
class GDict:
    ...
    def select_with_mask(self, mask, wrapper=True):
        return self._recursive_do(
            self.memory, select_with_mask, mask=mask, wrapper=wrapper, capacity=to_item(mask.sum())
        )
        ...
        if isinstance(memories[0], (tuple, list)):
            ...
            for i in range(len(memories)):
                assert set(memories[i].keys()) == set(
                    memories[0].keys()
                ), f"{set(memories[i].keys())}, {set(memories[0].keys())}"
            ret = {}
            for key in memories[0]:
                ret[key] = cls._do_on_list_of_array(
                    [memories[j][key] for j in range(len(memories))], function, **kwargs
                )
        else:
...
class DictArray(GDict):
    ...
    def shuffle(self, valid_capacity=None, wrapper=True, in_place=True):
        ...
        if in_place:
            ...
            return self._recursive_do(
                self.memory, take, indices=indices, axis=0, wrapper=wrapper, capacity=self.capacity
            )
...
class SharedGDict(GDict):
    ...
    def _create_np_from_memory(cls, shared_memory, shape, dtype):
        if isinstance(shared_memory, dict):
            ...
            memory = [
                cls._create_np_from_memory(shared_memory[k], shape[k], dtype[k]) for k in range(len(shared_memory))
            ]
        else:
