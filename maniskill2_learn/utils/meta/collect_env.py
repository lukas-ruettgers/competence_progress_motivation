...
def find_gpu():
    ...
    best_idx = 0
    best_used_memory = 10e100
    for idx, gpu in enumerate(res['gpu']):
        used_memory = gpu['fb_memory_usage']['total'] - gpu['fb_memory_usage']['free']
        free_memory = gpu['fb_memory_usage']['free']
        print(f"GPU {idx} has {free_memory:.2f}MB free memory and {used_memory:.2f}MB used memory!")
        if used_memory < best_used_memory and free_memory > 7000:
            best_used_memory = used_memory
            best_idx = idx
    print(f"Run Program on GPU {best_idx}!")
    return best_idx
