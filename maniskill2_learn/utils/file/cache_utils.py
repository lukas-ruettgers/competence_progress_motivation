    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.num_procs > 1:
            ...
            self.output_buffers = [
                SharedDictArray(self.cache_buffer.to_dict_array().slice(slice(0, 1))) for i in range(num_procs)
            ]
    ...
    def get_next_items(self, max_num, auto_restart=False):
        ...
        if filesuffix == "record":
            ...
            items = load_items_from_record(
                self.current_file, record_indices, self.item_index, self.item_index + num, True
            )
        ...
        **kwargs,
        ...
        if synchronized:
            self.worker = FileCacheWorker(
                filenames, capacity, keys, keys_map, buffer_infos, data_coder, num_procs, **kwargs
            )
        else:
