...
def get_kwargs_from_shape(obs_shape, action_shape):
    ...
    if isinstance(obs_shape, dict):
        ...
        visual_key = []
        if "xyz" in obs_shape:
            visual_key.append("pointcloud")
        else:
            visual_key += [name for name in obs_shape.keys() if name in PCD_KEYS or name in IMAGE_KEYS]

        visual_shape = obs_shape

        if "xyz" in visual_shape:
        ...
        if "rgb_image" in visual_key:
            # For new maniskill callback envs
            if len(visual_shape[visual_key]) == 3:
                num_images = 1
            else:
                assert (
                    len(visual_shape[visual_key]) == 4
                ), f"You need to provide either 3-dim or 5-dim inputs! The input shape is {visual_shape[visual_key]}!"
