...
class DistBlock(nn.Module):
    ...
    def forward(self, input):
        ...
        if self._dist == "mse":
            out = td.Normal(out, 1.0)
        elif self._dist == "normal":
            out = td.Normal(out, self._std)
        elif self._dist == "bernoulli":
            out = td.Bernoulli(out)
        return td.Independent(out, self._out_size)
...
class MLP(nn.Sequential):
    def __init__(
        ...
        for i in range(1, len(mlp_spec)):
            ...
            self.add_module(
                f"mlp_{i - 1}",
                build_nn_block(block_cfg, dict(in_features=in_features, out_features=mlp_spec[i])),
            )
