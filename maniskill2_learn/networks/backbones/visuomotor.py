...
class FrameMiners(ExtendedModule):
    def __init__(
        ...
        **kwargs,
        ...
        self.visual_nn = visual_nn or ExtendedModuleList([build_model(visual_nn_cfg) for i in range(num_frames)])
        if is_critic:
        ...
        **kwargs,
        ...
        if feature is None:
            ...
            for i in range(self.num_frames):
                ...
                pcd = {key: obs[key] for key in ["xyz", "rgb", "seg"] if key in obs}  # [B, N, 3]
                pcd["xyz"] = batch_transform_pos(to_frame, pcd["xyz"])
                frs_i_pos = batch_transform_pos(to_frame, frame_related_states).reshape(B, -1)  # [B, K * 3]
                if frame_goal_related_poses is not None:
                    frs_i_pose = batch_transform_pose(to_frame, frame_goal_related_poses).reshape(B, -1)
            ...
            feats = [feature[..., i * feat_dim : (i + 1) * feat_dim] for i in range(self.num_frames)]
        ...
        if self.is_critic:
            ...
            weight = self.fused_weight(torch.cat(feats, dim=-1)).reshape([B, self.num_frames, -1])  # [B, NF, NA]
