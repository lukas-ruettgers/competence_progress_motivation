...
from ..modules.spconv_modules import (
    initial_voxelize,
    voxel_to_point,
    point_to_voxel,
    ResidualBlock,
        ...
        **kwargs,
        ...
        for stage in range(len(cs)):
            if self.LAYERS[stage] > 0:
                self.net.add_module(
                    f"block_{stage}_0",
                    self.BLOCK(in_channel, cs[stage], kernel_size=3, stride=2, dilation=1, use_ln=use_ln),
                )
        ...
        if out_channel is not None:
            self.regression = nn.Sequential(
                nn.Linear(self.feat_dim, cs[-1] * 2), nn.ReLU(), nn.Linear(cs[-1] * 2, out_channel)
            )
        self.init_weights()
        if zero_init_output:
            m = self.regression[-1]
