...
class Evaluation:
    def __init__(
        ...
        **kwargs,
        ...
        log_level = (
            logging.INFO
            if (kwargs.get("log_all", False) or self.worker_id is None or self.worker_id == 0)
            else logging.ERROR
        )
        ...
        (
            ...
            self.episode_finishes,
            self.episode_succs,
            self.episode_succs_reached,
        ) = ([], [], [], [], [])
        self.episode_len, self.episode_reward, self.episode_finish, self.episode_succ, self.episode_succ_reached = (
            0,
            0,
            False,
            0,
    ...
    def start(self, work_dir=None):
        ...
        self.episode_succs, self.episode_succs_reached = [], []
        self.recent_obs = None
        self.data_episode = None
        self.video_writer = None
        self.level_index = -1
    ...
    def done(self):
        ...
        self.episode_succs.append(self.episode_succ / self.episode_len)
        self.episode_succs_reached.append(self.episode_succ_reached)
    ...
    def step(self, action):
        ...
        reward, done, info, episode_done, success = GDict(
            [infos["rewards"], infos["dones"], infos["infos"], infos["episode_dones"], infos["infos"]["success"]]
        ).item(wrapper=False)
        self.episode_len += 1
        self.episode_reward += float(reward)
        self.episode_succ += int(success)
        self.episode_succ_reached |= bool(success)
        ...
        if episode_done:
            ...
            if self.log_every_episode:
                self.logger.info(
                    f"Episode {self.episode_id} ends: Length {self.episode_len}, Reward: {self.episode_reward}, Early Stop or Finish: {done}, Success: {success}"
                )
    ...
    def run(self, pi, num=1, work_dir=None, **kwargs):
        ...
        while self.episode_id < num:
            ...
            with torch.no_grad():
                with pi.no_sync(mode="actor"):
                    action = pi(recent_obs, mode=self.sample_mode, dones=episode_done)
                    action = to_np(action)
            ...
            if episode_done:
                ...
                log_mem_info(self.logger)
        self.finish()
        return self.episode_lens, self.episode_rewards, self.episode_finishes, self.episode_succs, self.episode_succs_reached
