...
class ExtendedEnv(ExtendedWrapper):
    ...
    **ExtendedEnv should also be the outermost wrapper (except BufferAugmentedEnv)**
    ...
    def step(self, action, *args, **kwargs):
        ...
        return (
            obs,
            np.float32(reward * self.reward_scale),
            np.bool_(terminated),
            np.bool_(truncated),
            info,
        )
...
"""
class MaxStepVecEnv(ExtendedWrapper):
    ...
    def __init__(self, vec_env, max_steps=None):
    ...
    def step(self, *args, **kwargs):
        self.counter += 1
        if(self.max_steps is not None and self.counter > self.max_steps):
            self.counter = 0
            return super().reset()
...
class ObsWrapper(ExtendedWrapper):
    ...
    def step(self, *args, **kwargs):
        obs, *args = super().step(*args, **kwargs)
        return self.observation(obs), *args
...
@WRAPPERS.register_module()
class RGBDObservationWrapper(ObsWrapper):
    """Map raw textures (Color and Position) to rgb and depth."""

    def __init__(self, env, pop_image_keys=True, *args, **kwargs):
        super().__init__(env)
        self.observation_space = deepcopy(env.observation_space)
        self.pop_image_keys = pop_image_keys

    def observation(self, observation: dict):
        if self.pop_image_keys:
...
@WRAPPERS.register_module()
class PointCloudObservationWrapper(ObsWrapper):
    ...
    def update_observation_space(space: spaces.Dict):
        ...
        for cam_uid in image_space:
            ...
            cam_pcd_space["xyzw"] = spaces.Box(low=-np.inf, high=np.inf, shape=(h * w, 4), dtype=np.float32)

            # Extra keys
            if "Color" in cam_image_space.spaces:
                cam_pcd_space["rgb"] = spaces.Box(low=0, high=255, shape=(h * w, 3), dtype=np.uint8)
            if "Segmentation" in cam_image_space.spaces:
                cam_pcd_space["Segmentation"] = spaces.Box(low=0, high=(2**32 - 1), shape=(h * w, 4), dtype=np.uint32)
...
class ManiSkill2_ObsWrapper(ExtendedWrapper, ObservationWrapper):
    def __init__(
        ...
        self.obs_mode = self.obs_mode.split("+")
