...
n_replay_horizon_orig = 50 
n_replay_horizon_imag = 15 
...
agent_cfg = dict(
    ...
    goal_quality_extr_bound = 4.0, # NOTE (Lukas): How much worse than target extrinsic reward may goal trajectory be?
    ...
    dreamer_cfg=dict(
        ...
        nn_cfg=dict(
            ...
            dyn_hidden=200, # Original: 200, Mediocre: 150, Simple: 100
        ...
        train_cfg=dict(
            ...
            model_lr=3e-4, # Original: 3e-4, Mediocre: 7e-4, Fast: 1e-3
            value_lr=8e-5, # Original: 8e-5, Mediocre: 2e-4, Fast: 5e-4
            actor_lr=8e-5, # Original: 8e-5, Mediocre: 2e-4, Fast: 5e-4
            opt_eps=1e-5,
            grad_clip=100,
            value_grad_clip=100,
            actor_grad_clip=100,
...
train_cfg = dict(
    ...
    warm_steps=1_000,  # TODO (Lukas): Finetune
    n_eval=5_000,
    n_checkpoint=40_000,
...
env_cfg = dict(
    type="gym",
    env_name="TurnFaucet-v0", # "PickCube-v0", "PickSingleEGAD-v0", "PickSingleYCB-v0", "StackCube-v0", "TurnFaucet-v0"
    ignore_dones=True,
    horizon=horizon,
    # obs_mode="pointcloud",
    obs_mode="image",
    # TODO (Lukas): Check if these configs work with our custom image obs_mode.
    n_points=1200,
    n_goal_points=50,
...
recent_traj_replay_cfg = dict(
    type="ReplayMemory",
    # TODO (Lukas): Adjust capacity of replay_buffer. Is reset anyway after each update step.
    capacity=int(1 * n_procs_rollout * horizon * n_runs_in_episode),
)
rollout_cfg = dict(
    type="Rollout",
...
eval_cfg = dict(
    ...
    save_traj=False,
    save_video=True, # TODO (Lukas): Activate if model turns out to achieve something.
